...
export default function InputOrder() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        alert('Gagal memuat data pelanggan atau layanan.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Endpoint ini asumsikan akan dibuat di backend nantinya
      ...
      <header className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold tracking-tight text-slate-800">Input Order Baru</h1>
